...
path = Path('/tmp/peg-v0.2.1-analysis.json')
...
project-evidence-graph review examples/customer-change.json \
  ...
  --lifecycle \
  --markdown /tmp/peg-v0.2.1-review.md \
  --html /tmp/peg-v0.2.1-review.html
...
project-evidence-graph pack build examples/customer-change.json \
  ...
  --risk-policy examples/risk-policy.json \
  --lifecycle
project-evidence-graph pack verify /tmp/peg-v0.2.1-pack
```
